...
impl Blender {
    ...
    /// Parse standard output into version from running program's version arguments.
    ///
    /// # Errors
    /// * ExecutableInvalid - Invalid executable, unable to find version number, or data read from executable is malformed.
    /// * ParseInt - Unable to parse blender version explicitly.
    fn parse_version_from_stdout(output: Output) -> Result<Version, BlenderError> {
        static VERSION_REGEX: LazyLock<Regex> = LazyLock::new(|| {
            Regex::new(r"Blender (?<major>[0-9]).(?<minor>[0-9]).(?<patch>[0-9])").unwrap()
        });

        let stdout =
            String::from_utf8(output.stdout).map_err(|_| BlenderError::ExecutableInvalid)?;
        // .expect("Should be able to read string content from running this program!");

        let (major, minor, patch) = match VERSION_REGEX.captures(&stdout) {
            Some(cap) => {
                let (_, [major, minor, patch]) = cap.extract();
                (
                    Self::handle_parse(major)?,
                    Self::handle_parse(minor)?,
                    Self::handle_parse(patch)?,
                )
            }
            None => {
                eprintln!("Found no regex matches! {stdout:?}");
                return Err(BlenderError::ExecutableInvalid);
            }
        };
    ...
    /// This function will invoke the -(-v)ersion command to retrieve blender version information.
    ...
    ///
    /// # Errors
    /// * ExecutableNotFound - Executable in path could not be found - Verify the path provided exist and not compressed
    /// * ExecutableInvalid - Invalid executable, unable to find version number, or data read from executable is malformed
    /// * InvalidData - Executable is invalid.
    /// * IoErrors - Error generated by IO library. Mostly permission issue
    /// * ParseInt - Unable to parse blender version properly
    ...
    fn check_version(executable_path: impl AsRef<Path>) -> Result<Blender, BlenderError> {
        ...
        let version = Self::parse_version_from_stdout(output)?;
        Ok(Blender::new(exec_path, version))
    }
    ...
    fn verify_checksum(path: impl AsRef<Path>, data: &[u8]) -> Result<(), BlenderError> {
        let content = fs::read(&path).map_err(BlenderError::IoError)?;
        let source = Self::calculate_checksum(&data);
        let target = Self::calculate_checksum(&content);
        if source.ne(&target) {
            fs::write(path, data).map_err(BlenderError::IoError)?;
        }
        Ok(())
    }

    fn generate_script_path() -> Result<PathBuf, BlenderError> {
    ...
    fn get_script_path() -> Result<PathBuf, BlenderError> {
        let script_path = Self::generate_script_path()?;
        ...
        if script_path.exists() {
            Self::verify_checksum(&script_path, data)?;
        } else {
    ...
    fn invoke(&self, args: Args) -> Result<BufReader<ChildStdout>, BlenderError> {
        let col = &args.generate_arg_command(Self::get_script_path()?)?;
...
pub(crate) mod tests {
    ...
    */
    ...
    fn ensure_verify_checksum_overwrite_unmatched_checksum() {
        ...
        fs::write(&script_path, b"Test")
            .expect("Need to overwrite existing script data for unit test!");

        let original_checksum = Blender::calculate_checksum(
            &fs::read(&script_path)
        ...
        let data = include_bytes!("./render.py");
        let result = Blender::verify_checksum(&script_path, data);
        assert!(result.is_ok());
        assert_ne!(
            Blender::calculate_checksum(
                &fs::read(&script_path)
                    .expect("Should be able to read the file!")
                    .to_vec()
            ),
    ...
    #[test]
    fn ensure_get_script_path_generates_script_if_non_existing() {
        let script_path = Blender::generate_script_path();
        assert!(script_path.is_ok());
        let script_path = script_path.unwrap();

        fs::remove_file(&script_path).expect("Must be able to remove script file for unit test");
        let result = Blender::get_script_path();
    ...
    fn assure_get_script_path_succeed() {
        ...
        assert!(path.is_ok_and(|p| p.exists() && p.is_file()));
    }
